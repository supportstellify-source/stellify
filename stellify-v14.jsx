...
const C = {
  ...
  // ── GROQ CONFIG ──────────────────────────────
  GROQ_KEY: "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",
...
// ═══════════════════════════════════════════
// 🧠 AI BACKEND – Anthropic API (funktioniert in Vorschau + Produktion, kein CORS)
// ═══════════════════════════════════════════
...
const ANT_MODEL_FAST = "claude-haiku-4-5-20251001";
const ANT_MODEL_FULL = "claude-sonnet-4-6";
...
const getModel = (toolId) => HAIKU_TOOLS.includes(toolId) ? ANT_MODEL_FAST : ANT_MODEL_FULL;
...
// Anthropic API messages builder
function buildMessages(prompt, system) {
  if(typeof prompt === "string") return [{role:"user", content:prompt}];
  // Array von messages – filtere system raus (Anthropic hat system als separates Feld)
...
async function callAI(prompt, system, toolId="") {
  ...
  if(r.status===401) throw new Error("API-Schlüssel ungültig.");
...
Verhalten: Antworte konkret und umsetzbar (max. 3-4 Sätze im Widget). Schreib Beispieltexte direkt aus wenn gefragt. Empfehle Tool-Namen exakt wie oben damit Links funktionieren. Sei warm, direkt, wie ein erfahrener Karriere-Coach.`;
  ...
  const send = async () => {
    ...
    try {
      ...
      const reply = data.content?.[0]?.text || "Bitte nochmals versuchen.";
...
export default function App() {
  ...
  const sendIC=async()=>{if(!icIn.trim())return;if(!canGenPro())return;const um=icIn.trim();setIcIn("");const nc=icN+1;setIcN(nc);
    ...
    try{if(nc>=5){const h=msgs.map(m=>`${m.r==="ai"?"Interviewer":"Kandidat"}: ${m.t}`).join("\n");
      ...
      const r2=await fetch(ANTHROPIC_URL,{method:"POST",headers:ANTHROPIC_HEADERS,
        body:JSON.stringify({model:ANT_MODEL_FAST,max_tokens:400,system:t.coach.icNext(job),messages:history})});
