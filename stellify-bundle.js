...
const GenericToolPage = React.memo(function GenericToolPage({ tool, lang, pro, setPw, setPage, yearly, C, proUsage, setProUsage });) {
...
// SPLASH SCREEN
// ══════════════════════════════════════════════════════════
const SplashScreen = React.memo(function SplashScreen({ onDone });) {
        ...
        <div className="nl nl-desk">
          ...
          {authSession ? (
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              {authSession.plan==="ultimate"&&!authSession.isAdmin&&<button onClick={()=>setShowMembers(true)}
              ...
              {authSession&&<button onClick={()=>setShowReferral(true)}
                style={{padding:"5px 10px",borderRadius:8,border:"1px solid rgba(245,158,11,.3)",background:"rgba(245,158,11,.06)",color:"#f59e0b",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                🎁 {lang==="de"?"Freunde":lang==="fr"?"Amis":lang==="it"?"Amici":"Refer"}
              </button>}
...
if (rootEl) { const root = ReactDOM.createRoot(rootEl); root.render(React.createElement(React.StrictMode, null, React.createElement(App))); }
